...
public:
    ...
    void checkOut() {
        if (!isCheckedOut) {
            isCheckedOut = true;
            // set due date, for example, 14 days from now
...
int main() {
    ...
    library.addPatron("John Doe", 1234);
    library.addPatron("Jane Smith", 5678);
    ...
    library.returnBook("To Kill a Mockingbird", 5678);
